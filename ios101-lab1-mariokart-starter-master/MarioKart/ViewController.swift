...
class ViewController: UIViewController,
  ...
  @IBAction func didDoubleTapKart(_ sender: UITapGestureRecognizer) {
    ...
    // Tip: Use the `translate` function below
      ...
      }// moving the kart forward by the width of the containing view and translating it back to it's position
  ...
  private func moveKart(using gestureRecognizer: UIPanGestureRecognizer) {
      let location = gestureRecognizer.location(in: view) // get the gesture location
      let kartView = gestureRecognizer.view // get a reference to the kart
      kartView?.center = location // move the kart to the location
  }
  ...
  private func resetKarts() {
      UIView.animate(withDuration: 0.4) {
          ...
          //reset the positions of the karts to the initial positions
  ...
  private func raceKartsWithRandomizedSpeed() {
      let kartView0Speed = Double.random(in: 0.5...5) // Gets a random value from 0.5 to 5
      translate(kart: kartView0, by: view.frame.width, animationDuration: kartView0Speed)
      let karView1Speed = Double.random(in: 0.5...5)
      translate(kart: kartView1, by: view.frame.width, animationDuration: karView1Speed)
